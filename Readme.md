...
`esy-cargobootstrap` is sample package, [`hello`](https://github.com/esy/test-hello-c), packaged for [`esy`](https://esy.sh/).
...
## How to use `esy-cargobootstrap`?

`esy-cargobootstrap` can be used from both NPM and directly from Github.

### From NPM 

`esy-cargobootstrap` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-cargobootstrap` to install it, or specify it in
...
{
  "dependencies": {
    "esy-cargobootstrap": "esy-packages/esy-cargobootstrap"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-cargobootstrap": "esy-packages/esy-cargobootstrap#<commit hash>"
  }
```
